...
async def handle_task(request: Request) -> JSONResponse:
    ...
    try:
        try:
            result = await agent.run(task=payload, district_id=task["district_id"],
                                     run_ref=run_ref)
            ...
            # Release the dedup marker: the 500 below asks Pub/Sub to redeliver, and the
            # retry must not be swallowed as a duplicate of this failed attempt.
            dedup_ref.delete()
